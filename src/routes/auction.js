  ...
  try {
    ...
    try {
      ...
      if (!existingAuction.length) {
        const reservePrice = ethers.formatUnits(
...
router.post('/auctionResulted', service_auth, async (req, res) => {
  try {
    ...
    const winningBid = ethers.formatUnits(
      BigInt(winningBidBN.hex),
      auctionPayToken.decimals
    );
    const unitPrice = ethers.formatUnits(
      BigInt(unitPriceBN.hex),
      auctionPayToken.decimals
    );
