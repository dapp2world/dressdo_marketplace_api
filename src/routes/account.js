 ...
 *     description: Retrieve account information based on the provided address.
 *     tags:
 *       - account
 *     security:
 *       - BearerAuth: []
...
router.get("/getaccountinfo", auth, async (req, res) => {
  let address = await extractAddress(req);
  let account = await Account.findOne({ address: address });
  if (account) {
    return res.json({
      status: "success",
 ...
 *     description: Retrieve user account information based on the provided address.
 *     tags:
 *       - account
 *     parameters:
 *       - in: path
...
router.get("/nonce/:address", auth, async (req, res) => {
  try {
    ...
    let __account = await account.save();
    return res.json({
      status: "success",
